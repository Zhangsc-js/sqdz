<template>
  <div style="width: 100%;">
    <el-form inline ref="queryForm" :model="queryForm">
      ...
      <el-row>
        <el-form-item>
          <el-button v-has="'DEV-OPS-CHECK-PLAN-TEMP-ADD'" type="primary" @click="addCheckingTemp()" icon="el-icon-plus">新增</el-button>
          <el-button v-has="'DEV-OPS-CHECK-PLAN-TEMP-BATDELETE'" type="danger" :disabled="batchBtnVisibles" @click="batchDelete">批量删除</el-button>
        </el-form-item>
      </el-row>
    </el-form>
    ...
    <el-table stripe border :data="tableData" @selection-change="selsChange" style="width: 100%" height="500px">
      ...
      <el-table-column v-if="hasBtn([
        'DEV-OPS-CHECK-PLAN-TEMP-UPDATE',
        ...
        <template v-slot="scope">
          <el-button v-has="'DEV-OPS-CHECK-PLAN-TEMP-UPDATE'" type="text" size="small" @click="updateplan(scope.row.id)">更新</el-button>
          <el-button v-has="'DEV-OPS-CHECK-PLAN-TEMP-DELETE'" type="text" size="small" @click="deletePlan(scope.row.id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <Pagination
...
import CheckingPlanAdd from "./checking-plan-temp-add";
import CheckingPlanUd from "./checking-plan-temp-ud";
...
export default {
  ...
  methods: {
    ...
    addCheckingTemp() {
      ...
      this.title = "新增";
      this.dialogForm = CheckingPlanAdd;
    ...
    detailsPlan(id) {
      ...
      this.id = id;
      this.title = "详情";
      this.dialogForm = CheckingPlanUd;
    },
    updateplan(id) {
      this.disabled = false;
      this.dialogVisible = true;
