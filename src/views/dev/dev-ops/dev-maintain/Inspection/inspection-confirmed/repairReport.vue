<template>
  <div style="width: 100%">
    ...
    <el-button icon="el-icon-plus" href="javascript:void(0)" type="primary" :disabled="batchBtnVisable"  @click="report()">报修</el-button>
    <el-table stripe border :data="tableData" @selection-change="handleSelectionChange" style="width: 100%">
...
<script>
import { 
  getInsExceptionRecordItems, 
  reportInsExceptionRecord } from '@/api/device'
...
export default {
  ...
  methods: {
    getData(pageNum) {
      ...
      getInsExceptionRecordItems(params).then(response => {
    ...
    report() {
      reportInsExceptionRecord(this.selectData).then(response => {
        const result = response.data
        if (result.success) {
          // 最后一页全部报修后再次点检无数据bug
          this.page.pageNum = 1
