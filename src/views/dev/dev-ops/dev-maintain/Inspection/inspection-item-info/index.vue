  ...
  <div style="height: 100%">
    <el-form inline ref="queryForm" :model="queryForm" style="margin-left:20px">
      ...
      <el-row style="margin-top:10px">
        <el-form-item>
          <el-button
            v-has="'DEV-OPS-INSPECTION-ITEM-INFO-ADD'"
            type="primary"
            @click="addRow"
            icon="el-icon-plus"
          >新增</el-button>
          <el-button
            v-has="'DEV-OPS-INSPECTION-ITEM-INFO-DNOWLOAD'"
            type="primary"
          ...
          >
            <el-button
              v-has="'DEV-OPS-INSPECTION-ITEM-INFO-UPLOAD'"
              type="primary"
              icon="el-icon-upload2"
            >批量导入</el-button>
          </el-upload>
          <el-button
            v-has="'DEV-OPS-INSPECTION-ITEM-INFO-BATDELETE'"
            type="danger"
    ...
    >
      ...
      <el-table-column prop="itemInfoNo" label="巡检项信息编号" align="center">
        <template v-slot="scope">
      ...
      <el-table-column
        v-if="hasBtn([
          'DEV-OPS-INSPECTION-ITEM-INFO-UPDATE',
          'DEV-OPS-INSPECTION-ITEM-INFO-DELETE'
        ])"
        align="center"
        label="操作"
        fixed="right"
      ...
      >
        <template v-slot="scope">
          <el-button
            v-has="'DEV-OPS-INSPECTION-ITEM-INFO-UPDATE'"
            type="text"
          ...
          >取消</el-button>
          <el-button
            v-has="'DEV-OPS-INSPECTION-ITEM-INFO-DELETE'"
            type="text"
...
import {
  getInspectionItemInfos,
  getDevTree,
  updateInspectionItemInfo,
  addInspectionItemInfo,
  deleteInspectionItemInfo,
  batchDelInspectionItemInfos,
  downloadInspectionItemTemp,
...
export default {
  ...
  data() {
    return {
      ...
      fileUploadUrl: IMPORT_INSPECTION_ITEMS,
      queryParams: {
        isChecking: 1
      },
      token: {
  ...
  methods: {
    ...
    batchDel() {
      ...
      this.$confirm("此操作将永久删除该记录, 是否继续?", "提示", {
        ...
        .then(() => {
          batchDelInspectionItemInfos(this.selectIds).then(response => {
            const result = response.data;
            if (result.success) {
    ...
    deleteRow(id) {
      this.$confirm("此操作将永久删除该记录, 是否继续?", "提示", {
        ...
        .then(() => {
          deleteInspectionItemInfo(id).then(response => {
            const result = response.data;
            if (result.success) {
    ...
    formValidate(row) {
      ...
      if (!row.itemInfoNo) {
        this.$message.error("请填写巡检项编码");
        return true;
      }
      if (!row.partsName) {
        this.$message.error("请填写巡检部位");
        return true;
      }
      if (!row.projectName) {
        this.$message.error("请填写巡检内容");
        return true;
      }
      if (!row.methodName) {
        this.$message.error("请填写巡检方法");
        return true;
      }
      if (!row.criteriaName) {
        this.$message.error("请填写巡检标准");
        return true;
      }
      return false;
    }
