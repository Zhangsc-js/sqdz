<template>
  <div style="width: 100%;">
    <el-form ref="queryForm" :model="queryForm" style="margin-left:15px" inline>
      <el-row>
      ...
      <el-row>
        <el-form-item>
          <el-button
            v-has="'DEV-OPS-LUBRICATION-PLAN-ADD'"
            type="primary"
            @click="addLubricationPlan()"
            icon="el-icon-plus"
          >新增</el-button>
          <!-- <el-button v-has="'DEV-OPS-LUBRICATION-PLAN-BATDELETE'" :disabled="batchBtnVisibles" type="danger" @click="batchDelete">批量删除</el-button> -->
        </el-form-item>
      </el-row>
    </el-form>
    ...
    >
      ...
      <el-table-column
        v-if="hasBtn([
          'DEV-OPS-LUBRICATION-PLAN-ACTIVATE',
          'DEV-OPS-LUBRICATION-PLAN-UPDATE',
          'DEV-OPS-LUBRICATION-PLAN-DELETE',
          'DEV-OPS-LUBRICATION-PLAN-STOP'
        ])"
        align="center"
        label="操作"
        width="220px"
      >
        <template v-slot="scope">
          <el-button
            v-has="'DEV-OPS-LUBRICATION-PLAN-ACTIVATE'"
          ...
          <el-button
            v-has="'DEV-OPS-LUBRICATION-PLAN-UPDATE'"
          ...
          >更新</el-button>
          <el-button
            v-has="'DEV-OPS-LUBRICATION-PLAN-DELETE'"
            v-if="scope.row.status === 0"
            type="text"
            size="small"
          ...
          >删除</el-button>
          <el-button
            v-has="'DEV-OPS-LUBRICATION-PLAN-STOP'"
...
import {
  getLubricationPlan,
  deleteLubricationPlan,
  batchDelLubricationPlan,
    ...
    deletePlan(id) {
      this.$confirm("此操作将永久删除该记录, 是否继续?", "提示", {
        ...
        .then(() => {
          ...
          deleteLubricationPlan(id).then(response => {
            const result = response.data;
            if (result.success) {
    ...
    batchDelete() {
      this.$confirm("此操作将永久删除该记录, 是否继续?", "提示", {
        ...
        .then(() => {
          ...
          batchDelLubricationPlan(this.sels)
            .then(response => {
              const result = response.data;
              if (result.success) {
                this.$message.success("删除成功");
