  ...
  <div>
    ...
    <div>
      <div class="padding20 pt12" v-if="!disabled">
        ...
        <el-button type="danger" v-if="!disabled" :disabled="batchBtnVisibles" @click="batchDelete">批量删除</el-button>
      </div>
      ...
      <el-divider content-position="center">设备润滑项目</el-divider>
      <el-table stripe border highlight-current-row :data="tableData" style="width: 100%">
        ...
        <el-table-column prop="item.partsName" label="润滑部位"></el-table-column>
        <el-table-column prop="item.methodName" label="润滑方法"></el-table-column>
        <el-table-column prop="item.lubricationNo" label="润滑油脂牌号"></el-table-column>
        <el-table-column fixed="right" align="center" label="操作" width="170px">
          <template v-slot="scope">
            <el-button v-if="!disabled" type="text" size="small" @click="deleteLubricationPlanTempItem(scope.row.id)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
<script>
    import lubricationItemAdd from './temp-item-add'
    import Pagination from '@/components/Pagination'
    import {
        getLubricationTempItems,
        deleteLubricationPlanTempItem,
        batchDelLubricationTempItems,
        getLubricationTempById,
        updateLubricationTemp,
        getLubTempGroupByDev,
        deleteByLubDevNo } from '@/api/device'
    ...
    export default {
        ...
        methods: {
            getData() {
                getLubricationTempById(this.tempId).then(response => {
            ...
            getTableData(row) {
                ...
                getLubricationTempItems(params).then(response => {
            ...
            getDevData() {
                ...
                getLubTempGroupByDev(params).then(response => {
                    const result = response.data
                    if (result.success) {
            ...
            deleteByDevNoAndTempNo(row) {
                ...
                this.$confirm('此操作将永久删除该记录及其子记录, 是否继续?', '提示', {
                    ...
                    deleteByLubDevNo(params).then(response => {
                    const result = response.data
                    if (result.success) {
            ...
            save() {
                this.$refs['updateForm'].validate(valid => {
                    if (valid) {
                        updateLubricationTemp(this.updateForm).then(response => {
                            const result = response.data
                            if (result.success) {
            ...
            addCheckingTempItem() {
                ...
                this.dialogComponent = lubricationItemAdd
                this.title = '选择设备'
            },
            deleteLubricationPlanTempItem(id) {
                this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    deleteLubricationPlanTempItem(id).then(response => {
                        const result = response.data
                        if (result.success) {
                            this.$message.success('删除成功!')
                            this.getTableData()
            ...
            batchDelete() {
                this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
                    ...
                    batchDelLubricationTempItems(this.sels).then(response => {
                        const result = response.data
                        if (result.success) {
                            this.$message.success('删除成功!')
                            this.getTableData()
