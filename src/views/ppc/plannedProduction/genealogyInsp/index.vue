<template>
  <div style="width: 100%; height: 100%">
    <el-form inline ref="queryForm" :model="queryForm" label-width="100px">
      <el-row>
        ...
        <el-form-item>
          ...
          <el-button icon="el-icon-download" type="primary" @click="exportGenealogyInsp">导出</el-button>
          <el-button
      ...
      <el-row v-show="showMore">
        ...
        <el-form-item label="质检工序" prop="inspectProcessCode">
          <el-select v-model="queryForm.inspectProcessCode" @change="processChange" clearable>
            <el-option
    ...
    >
      ...
      <el-table-column prop="inspectProcessCode" label="质检工序" align="center" width="'10%'"></el-table-column>
      ...
      <el-table-column prop="inspectResult" label="质检结果" :formatter="cellFormat" align="center" width="'10%'"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      getInspStatus().then(res => {
        const result = res.data;
        if (result.success) {
          this.inspStatus = result.data;
        } else {
          this.$message.error(result.message);
        }
      })
    ...
    getData() {
      ...
      getGenealogyInsp(param).then(res => {
        const result = res.data;
        if (result.success) {
    ...
    exportGenealogyInsp() {
      ...
      exportGenealogyInsp(param).then(res => {
        const result = res.data;
        if (result) {
          let data = new File([result], { type: "application/octet-stream" });
          saveAs(data, "质检谱系过站信息.xls");
        } else {
          this.$message.error(result.message);
        }
      });
    ...
    cellFormat(r, c, v) {
      if (isEmptyArray(this.inspStatus) || isEmpty(v)) {
        return v;
      }
      console.log(this.inspStatus)
      console.log(v)
      const el = this.inspStatus.find(e => e.code == v);
