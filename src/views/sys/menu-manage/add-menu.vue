...
<script>
import { addMenu } from "@/api/sys";
export default {
  name: "add-menu",
  data() {
    return {
      addInput: { meta: { title: "", icon: "dashboard", requiresArg: false } }
    };
  },
  props: {
    selData: {
  ...
  methods: {
    setInput() {
      this.addInput = {
        ...
        isExternal: 0,
        code: "",
        meta: { title: "", icon: "dashboard", requiresArg: false,keepAlive:false }
      };
    },
    append() {
      let newChild = this.addInput;
      newChild.pid = this.selData.id;
      let data = { ...newChild };
