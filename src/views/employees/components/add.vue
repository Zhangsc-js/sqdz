<template>
  <div class="add-form">
    <el-dialog title="编辑员工" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :model="formBase" label-position="left" label-width="120px" style='margin-left:30px; width:337px;'>
...
<script>
import { add, jobnumber } from '@/api/employees'
    ...
    createData() {
      add(this.formBase).then(res => {
        ...
        if(res.data.success) {
          ...
          this.$emit('doQuery', {})
          //this.$refs['dataForm'].resetFields()//完成后清空数据表，待优化
        }
      })
    },
